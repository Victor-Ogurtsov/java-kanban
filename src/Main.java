...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        Task newTask1 = new Task("Приготовить завтрак", "Приготовить кашу, чай зеленый с сахаром, накрыть на сgeтол");
        System.out.println(inMemoryTaskManager.addTask(newTask1));
        Task newTask2 = new Task("Прибраться на кухне", "Помыть посуду, протереть пол, убрать пыль");
        System.out.println(inMemoryTaskManager.addTask(newTask2));

        System.out.println("Создание двух сложных Epic задач:");
        Epic newEpic1 = new Epic("Подарок к НГ", "Выбрать, купить, подарить подарок");
        System.out.println(inMemoryTaskManager.addEpic(newEpic1));
        Epic newEpic2 = new Epic("Выбрать бригаду для ремонта", "Сравнить сметы нескольких бригад по ремонту");
        System.out.println(inMemoryTaskManager.addEpic(newEpic2));
        ...
        System.out.println(inMemoryTaskManager.addSubTask(newEpic1.getId(), newSubTask1));
        SubTask newSubTask2 = new SubTask("Забрать и вручить подарок", "Забрать подарок по адресу... Вручить.");
        System.out.println(inMemoryTaskManager.addSubTask(newEpic1.getId(), newSubTask2));
        ...
        System.out.println(inMemoryTaskManager.addSubTask(newEpic2.getId(), newSubTask3));
        ...
        Task taskToUpdate = inMemoryTaskManager.getTask(newTask1.getId());
        ...
        System.out.println(inMemoryTaskManager.getTask(taskToUpdate.getId()));

        System.out.println("Изменение названия в Epic:");
        Epic epicToUpdate = inMemoryTaskManager.getEpic(newEpic1.getId());
        epicToUpdate.setName("Подарок на 8 марта");
        inMemoryTaskManager.updateEpic(epicToUpdate);
        System.out.println("Вывод Epic по идентификатору:");
        System.out.println(inMemoryTaskManager.getEpic(epicToUpdate.getId()));

        System.out.println("Изменение статуса в подзадаче:");
        SubTask subTaskToUpdate = inMemoryTaskManager.getSubtask(newSubTask1.getId());
        ...
        System.out.println(inMemoryTaskManager.getSubtask(subTaskToUpdate.getId()));
        System.out.println("Вывод Epic, статус изменился после обновления подзадачи:");
        System.out.println(inMemoryTaskManager.getEpic(newEpic1.getId()));
        ...
        System.out.println(inMemoryTaskManager.getAllSubTaskOfEpic(newEpic1.getId()));

        System.out.println("Удаление задачи по идентификатору");
        inMemoryTaskManager.removeTask(newTask1.getId());
        System.out.println("Удаление Epic по идентификатору");
        inMemoryTaskManager.removeEpic(newEpic2.getId());
